...
const Bubble = ({ bubble, handlePop, removeBubble }: any) => {
  ...
  return (
    <motion.div
      ...
      onClick={() => handlePop(currentText, bubble.id)}
...
export default function PopTheWordStage2() {
  ...
  const restartGame = () => {
    setScore(0); setLives(3); setBubbles([]); setGameOver(false); setVictory(false);
    setTargetWord(WORDS_LIST[Math.floor(Math.random() * WORDS_LIST.length)]);
  };
  ...
  return (
    <div className="relative w-full h-[calc(100vh-100px)] min-h-[500px] bg-gradient-to-b from-indigo-300 via-purple-100 to-white dark:from-slate-950 dark:via-slate-900 dark:to-slate-800 overflow-hidden rounded-[30px] border border-slate-200 dark:border-slate-700 shadow-inner">
      ...
      <div className="relative w-full h-full z-10">
        <AnimatePresence>
          {bubbles.map((bubble) => (
            <Bubble key={bubble.id} bubble={bubble} handlePop={handlePop} removeBubble={removeBubble} />
          ))}
        </AnimatePresence>
      </div>
    </div>
