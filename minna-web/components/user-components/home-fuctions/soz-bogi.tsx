...
import { Lightbulb, RotateCcw, Trophy, Star, Sparkles, Home } from 'lucide-react';
import { LEVELS } from './soz-levels';

// Yangi yaratilgan menyuni import qilamiz
import MainMenu from './soz-Menu';

const WordGardenGame = () => {
  // === O'YIN HOALTI VA XOTIRA (LOCALSTORAGE) ===
  ...
  // 1. BRAUZER XOTIRASIDAN SAQLANGAN DARAJANI O'QISH
  useEffect(() => {
    const savedLevel = localStorage.getItem('wordGardenHighestLevel');
    const savedCoins = localStorage.getItem('wordGardenCoins');
    
    if (savedLevel) setHighestLevel(parseInt(savedLevel));
    if (savedCoins) setCoins(parseInt(savedCoins));
  }, []);

  // 2. YANGI DARAJA OCHILGANDA XOTIRAGA SAQLASH
  useEffect(() => {
    localStorage.setItem('wordGardenHighestLevel', highestLevel.toString());
    localStorage.setItem('wordGardenCoins', coins.toString());
  }, [highestLevel, coins]);

  // MOBIL UCHUN GLOBAL SKROLLNI O'CHIRISH
  useEffect(() => {
  ...
  const checkWord = () => {
    ...
    if (currentLevel.targetWords.includes(currentString) && !foundWords.includes(currentString)) {
      ...
      if (foundWords.length + 1 === currentLevel.targetWords.length) {
        ...
        // Agar eng oxirgi ochilgan bosqichdan yutsa, yangisini ochadi
        if (levelIdx + 1 === highestLevel && highestLevel < LEVELS.length) {
  ...
  const handleHint = () => {
    if (coins < 30 || !currentLevel) return;
    ...
    currentLevel.targetWords.forEach((word, wIdx) => {
      if (!foundWords.includes(word)) {
        word.split("").forEach((char, cIdx) => {
          ...
          if (!alreadyHinted) unrevealed.push({ wIdx, cIdx, char });
        });
      }
    });
  ...
  const startGame = (targetLevelIndex: number = highestLevel - 1) => {
    setLevelIdx(targetLevelIndex);
    resetLevel();
  ...
  if (gameState === 'menu') {
    return <MainMenu onStart={startGame} highestLevel={highestLevel} totalLevels={LEVELS.length} />;
  }
      ...
      <div className="relative z-10 flex flex-col h-full w-full max-w-5xl mx-auto overflow-hidden">
        ...
        <div className="flex items-center justify-between p-3 sm:p-6 pt-6 sm:pt-6 h-[12%] min-h-[75px] bg-white/10 dark:bg-black/20 backdrop-blur-md rounded-b-3xl border-b border-white/20">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {/* HOME TUGMASI (Menyuga qaytish) */}
            <button 
              onClick={() => setGameState('menu')}
              className="p-2 sm:p-3 bg-white/50 dark:bg-white/10 rounded-xl hover:bg-blue-500 hover:text-white transition-all shadow-sm active:scale-95 text-slate-700 dark:text-slate-300"
            >
              <Home size={20} />
            </button>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button onClick={handleHint} disabled={coins < 30} className={`p-2 sm:p-3 rounded-xl transition-all shadow-sm ${coins >= 30 ? 'bg-white/50 dark:bg-white/10 hover:bg-yellow-400 hover:text-white text-yellow-600 dark:text-yellow-400 active:scale-90 cursor-pointer' : 'bg-white/20 dark:bg-white/5 text-slate-400 dark:text-slate-600 opacity-50 cursor-not-allowed'}`}>
              <Lightbulb size={20} />
            </button>
            <div className="flex items-center gap-1.5 sm:gap-2 bg-gradient-to-r from-amber-400 to-yellow-500 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl border-b-2 border-orange-700 shadow-lg">
              ...
              <motion.span key={coins} initial={{ scale: 1.5, color: "#fcd34d" }} animate={{ scale: 1, color: "#fff" }} className="font-black text-white text-sm sm:text-base min-w-[30px] sm:min-w-[40px] text-center">{coins}</motion.span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-evenly py-4 sm:py-6 max-md:pb-16 overflow-hidden px-4">
              ...
              return (
                <div key={wIdx} className="flex gap-1.5 bg-white/10 dark:bg-black/30 p-2.5 rounded-2xl backdrop-blur-md border border-white/10">
                  {word.split("").map((char, cIdx) => {
                    ...
                    return (
                      <motion.div key={cIdx} animate={{ scale: isFound ? [1, 1.1, 1] : 1, backgroundColor: isFound ? "#10b981" : (isHinted ? "rgba(250, 204, 21, 0.2)" : "rgba(255,255,255,0.05)") }} className={`w-11 h-11 sm:w-13 sm:h-13 flex items-center justify-center text-2xl font-black rounded-xl border-2 transition-all duration-500 ${isFound ? 'border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.5)]' : (isHinted ? 'border-yellow-400 shadow-[0_0_10px_rgba(250,204,21,0.4)]' : 'border-white/10')}`}>
                        <span className={isVisible ? (isHinted && !isFound ? "text-yellow-400" : "text-white") : "text-transparent"}>{char}</span>
                      </motion.div>
                    );
                  })}
                </div>
          ...
          <div className="h-16 flex items-center justify-center">
            <AnimatePresence>
              {currentString && (
                <motion.div initial={{ scale: 0.8, opacity: 0, y: 10 }} animate={{ scale: 1.1, opacity: 1, y: 0 }} exit={{ scale: 0.5, opacity: 0 }} className="px-12 py-3 bg-white/20 dark:bg-blue-600/30 backdrop-blur-xl text-slate-900 dark:text-white text-3xl font-black rounded-3xl shadow-2xl border border-white/30">
                  {currentString}
                </motion.div>
              )}
            </AnimatePresence>
          ...
          {/* HARF AYLANASI */}
          <div className="relative w-full max-w-[280px] sm:max-w-[340px] aspect-square flex items-center justify-center touch-none select-none" ref={containerRef} onTouchMove={handleTouchMove} onTouchEnd={checkWord}>
            ...
            <div className="relative w-56 h-56 sm:w-72 sm:h-72 bg-white/40 dark:bg-slate-900/40 rounded-full shadow-2xl border-4 border-white/60 backdrop-blur-2xl flex items-center justify-center">
              {currentLevel.letters.map((letter, index) => {
                ...
                return (
                  <div key={index} id={`letter-${index}`} onPointerDown={(e) => {e.preventDefault(); handleInput(index);}} onPointerEnter={(e) => e.buttons === 1 && handleInput(index)} onPointerUp={checkWord} style={{ transform: `translate(${x}px, ${y}px)` }} className={`absolute w-14 h-14 sm:w-18 sm:h-18 rounded-[20px] flex items-center justify-center text-3xl font-black cursor-pointer shadow-xl transition-all ${selectedIndices.includes(index) ? 'bg-blue-500 text-white border-blue-300 scale-110' : 'bg-white dark:bg-slate-800 text-slate-800 dark:text-white border-b-4 border-slate-200 dark:border-slate-700'}`}>
                    {letter}
                  </div>
                );
              })}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 z-[100] flex items-center justify-center p-6 bg-black/90 backdrop-blur-xl">
            <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="bg-white dark:bg-slate-800 p-10 rounded-[3.5rem] w-full max-w-sm text-center shadow-3xl border-t-8 border-yellow-400">
              ...
              <div className="flex gap-3">
                <button 
                  onClick={() => setGameState('menu')}
                  className="w-[30%] py-5 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-800 dark:text-white rounded-2xl font-black text-xl shadow-xl active:scale-95 transition-all flex justify-center items-center"
                >
                  <Home size={28} />
                </button>
                <button 
                  ...
                  className="w-[70%] py-5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl font-black text-2xl shadow-xl active:scale-95 transition-all"
                >
