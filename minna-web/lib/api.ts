...
export const setAuthToken = (token: string | null) => {
  globalToken = token;
  // Brauzerda ekanligimizni tekshiramiz (Next.js SSR xatosini oldini olish uchun)
...
const apiClient = axios.create({
  ...
  // withCredentials: true,  // ❌ Bearer token bilan kerak emas, agar CORS nastroyka to'g'ri bo'lsa
  headers: { 'Content-Type': 'application/json' },
});

apiClient.interceptors.request.use((config) => {
  // Avval globalTokenni tekshiramiz
  let token = globalToken;
  
  // Agar xotirada token bo'lmasa va brauzerda bo'lsak, localStoragedan qidiramiz
...
export const userAPI = {
  ...
  // streak calendar (Backendga moslandi)
