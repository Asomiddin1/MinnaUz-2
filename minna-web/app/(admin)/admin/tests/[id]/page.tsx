...
export default function TestDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const processedSections = (
        Array.isArray(testData.sections) ? testData.sections : []
      ).map((section: Section) => ({
        ...section,
  ...
  const openModal = (sectionId: number, question: Question | null = null) => {
    if (question) {
      ...
      const optionsForm: OptionFormItem[] = (question.options || []).map(
        (opt) => ({
          text: opt.text || "",
          image: null,
  ...
  if (error || !test)
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 p-8">
        ...
        <p className="font-medium text-red-600">{error || "Test topilmadi"}</p>
        <Button variant="outline" onClick={() => router.push("/admin/tests")}>
          <ArrowLeft className="mr-2 h-4 w-4" /> Testlarga qaytish
        </Button>
      </div>
    )
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 p-4 md:p-8">
      {/* TEST HEADER */}
      <div className="flex flex-col items-start justify-between gap-4 rounded-xl border bg-white p-6 shadow-sm sm:flex-row sm:items-center dark:bg-slate-900">
      ...
      {/* BO'LIMLAR */}
      {sections.length === 0 ? (
        <div className="rounded-xl border border-dashed bg-white py-16 text-center dark:bg-slate-900">
          <p className="text-lg text-slate-400">Hali bo'limlar mavjud emas</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-10">
          {sections.map((section) => (
            <div key={section.id} className="space-y-4">
              <div className="flex items-center justify-between rounded-lg border bg-slate-100 p-4 dark:bg-slate-800">
                              ...
                              {(q.options || []).map((opt, idx) => (
                                ...
                                >
                                  ...
                                  {String(idx) === q.correct_answer && (
                                    <Badge className="flex h-4 w-4 items-center justify-center rounded-full bg-green-500 p-0 text-[10px] text-white">
                                      ✓
                                    </Badge>
                                  )}
                                </div>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[850px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Barcha variantlarni matn yoki rasm bilan to'ldiring.
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 py-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, mondai_number: +e.target.value })
                  }
                />
              </div>
              <div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                className="min-h-[100px] resize-y" // O'qishga qulay bo'lishi uchun minimal balandlik berildi
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-4">
                ...
                {formData.question_image_preview && (
                  ...
                  >
                    <X className="mr-1 h-4 w-4" /> Rasmni o'chirish
                  </Button>
                )}
              </div>
              {formData.question_image_preview && (
                <div className="mt-2 h-32 w-40 overflow-hidden rounded border bg-gray-100 dark:bg-gray-800">
                  <img
                    src={formData.question_image_preview}
                    alt="preview"
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {formData.options.map((opt, idx) => (
                  <div
                    key={idx}
                    className="relative space-y-2 rounded-lg border bg-slate-50 p-3 dark:bg-slate-800"
                  >
                    <div className="flex items-center justify-between">
                      <span className="rounded bg-gray-200 px-2 py-0.5 text-xs font-bold dark:bg-gray-700">
                        Variant {idx + 1}
                      </span>
                      {opt.preview && (
                        <Button
                    ...
                    />
                    <label className="inline-flex cursor-pointer items-center gap-1 text-sm text-blue-600 hover:underline">
                      <ImagePlus className="h-4 w-4" />
                    ...
                    {opt.preview && (
                      <div className="h-24 w-full overflow-hidden rounded border bg-gray-100 dark:bg-gray-900">
                        <img
                          src={opt.preview}
                          alt={`var ${idx + 1}`}
                          className="h-full w-full object-contain"
            ...
            <DialogFooter>
              ...
              <Button
                ...
                className="min-w-[120px] bg-blue-600 text-white"
              >
